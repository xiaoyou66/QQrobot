...
async def parse_command(bot: NoneBot,ctx :Context_T,
    ...
    if not full_command:
        # command is empty
        await send(bot,ctx,"我不是智能音响，不需要唤醒哦，有问题就直接以小白开头我就可以回答你啦！")
        return None, None
...
async def handle_command(bot: NoneBot,ctx: Context_T) -> bool:
    ...
    """
    cmd, current_arg = await parse_command(bot,ctx, str(ctx['message']).lstrip())
